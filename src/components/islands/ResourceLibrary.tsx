...
import { ArrowRight, ArrowUp, BookOpen, Check, ChevronDown, Clock3, Download, FileText, FolderOpen, Search, X } from 'lucide-react';
...
const DownloadContext = createContext<(file: LibraryFile) => void>(() => {});

function FileCard({ file }: { file: LibraryFile }) {
  const onDownload = useContext(DownloadContext);
    ...
    <li className="library-file-item">
      ...
      <div className="library-file-actions">
        <a
          ...
          href={resourceUrl(file, 'download')}
          target="_blank"
...
export default function ResourceLibrary({ subjects: initialSubjects = [] }: { subjects?: LibrarySubject[] }) {
  ...
  const [downloadFile, setDownloadFile] = useState<LibraryFile | null>(null);
  ...
  return (
    <DownloadContext.Provider value={setDownloadFile}>
      ...
      <DownloadModal file={downloadFile} onClose={() => setDownloadFile(null)} />
